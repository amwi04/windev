...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	static BOOL go_from_left_right = TRUE;
	...
	switch (iMsg)
	{
		case WM_PAINT:
			...
			left_top_x = 0+xAnimated;
			left_top_y = (WINDOW_HEIGHT/2) - (ELLIPSE_SIZE_X/2);
			right_bottom_x = ELLIPSE_SIZE_X+xAnimated;
			right_bottom_y=(WINDOW_HEIGHT/2) + (ELLIPSE_SIZE_X/2);
		...
		case WM_TIMER:
			...
			if (go_from_left_right == TRUE)
			{
				xAnimated = xAnimated + 1;
				if (right_bottom_x == WINDOW_WIDTH)
				{
					go_from_left_right = FALSE;
				}
			}
			else
			{
